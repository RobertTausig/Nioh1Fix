...
readonly cmake_version="${NIOH1FIX_CMAKE_VERSION}"
...
readonly cmake_directory="${cache_directory}/cmake-${cmake_version}"
...
if [[ ! -x "${cmake_binary}" ]]; then
    extraction_directory="$(mktemp -d "${cache_directory}/.cmake-${cmake_version}.XXXXXX")"
...
if [[ "${actual_version}" != "${cmake_version}" ]]; then
    printf 'Expected CMake %s, found %s\n' \
        "${cmake_version}" "${actual_version}" >&2
    exit 1
fi
